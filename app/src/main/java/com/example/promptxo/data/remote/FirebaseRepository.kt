        ...
        try {
            val listener = firestore.collection("Post")
                .addSnapshotListener { snapshot, error ->
                    if (error != null) {
                        Log.w("FirebaseRepo", "Listen failed, using default posts", error)
                        trySend(defaultHomePosts.sortedByDescending { it.createdAt })
            ...
            Log.e("FirebaseRepo", "Error initializing firestore collection Post", e)
            trySend(defaultHomePosts.sortedByDescending { it.createdAt })
            awaitClose { }
        }
    }
    ...
    fun getImagePostsFlow(): Flow<List<ImagePostItem>> = callbackFlow {
        try {
            val listener = firestore.collection("imagepost")
                .addSnapshotListener { snapshot, error ->
                    if (error != null) {
                        Log.w("FirebaseRepo", "Listen failed on imagepost, using default images", error)
                        trySend(defaultImagePosts.sortedByDescending { it.createdAt })
                        return@addSnapshotListener
                    }
                    if (snapshot != null && !snapshot.isEmpty) {
            ...
            Log.e("FirebaseRepo", "Error initializing imagepost collection", e)
            trySend(defaultImagePosts.sortedByDescending { it.createdAt })
            awaitClose { }
        }
    }
